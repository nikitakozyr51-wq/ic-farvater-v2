// Каталог продуктов ИК Фарватер
// Данные из Google Spreadsheet — 767 продуктов, 5 категорий
// Сгенерировано автоматически из all_sheets.json

const PRODUCTS = [
  {
    "id": 1,
    "name": "ET1310PN1U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1310PN1U",
    "image": "../assets/images/products/items/ET1310PN1U.webp",
    "specs": {
      "Тип корпуса": "SOT23, SOT-224",
      "Тип": "Управление питанием (Power management)",
      "Входное напряжение": "2,5–5,5 В",
      "Выходное напряжение": "регулируемое",
      "КПД": "до 97%",
      "Ток холостого хода": "не более 50 мкА",
      "Рабочая частота": "1,3 МГц",
      "Температурный диапазон": "-40…+85 °C"
    }
  },
  {
    "id": 2,
    "name": "ET1309EN3.3T",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1309EN3.3T",
    "image": "../assets/images/products/items/ET1309EN3.3T 1.webp",
    "specs": {
      "Тип корпуса": "TO-263-3",
      "Тип": "Управление питанием (Power management)",
      "Входное напряжение": "15 В",
      "Выходное напряжение": "3,3 В",
      "Температурный диапазон": "-40…+85 °C"
    }
  },
  {
    "id": 3,
    "name": "ET1309ES035",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1309ES035",
    "image": "../assets/images/products/items/550_420__1309ES035 1.webp",
    "specs": {
      "Тип корпуса": "SOT-23, TO-92, SOT-89, SOP-8, TO-92L",
      "Тип": "Управление питанием (Power management)",
      "Входное напряжение": "2,495–36 В",
      "Выходное напряжение": "фиксируемое и регулируемое",
      "Температурный диапазон": "-40…+85 °C",
      "Напряжение питания": "до 36 В"
    }
  },
  {
    "id": 4,
    "name": "ET1309ES055",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1309ES055",
    "image": "../assets/images/products/items/550_420__1309ES035 1.webp",
    "specs": {
      "Тип корпуса": "SOT-23, TO-92, SOT-89, SOP-8, TO-92L",
      "Тип": "Управление питанием (Power management)",
      "Входное напряжение": "2,495–36 В",
      "Выходное напряжение": "фиксируемое и регулируемое",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "до 36 В"
    }
  },
  {
    "id": 5,
    "name": "ET1309ES065",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1309ES065",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "SOT-23, TO-92, SOT-89, SOP-8, TO-92L",
      "Тип": "Управление питанием (Power management)",
      "Входное напряжение": "2,495–36 В",
      "Выходное напряжение": "фиксируемое и регулируемое",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "до 36 В"
    }
  },
  {
    "id": 6,
    "name": "ET1645RU1U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1645RU1U",
    "image": "../assets/images/products/items/550_420__ET1645RU1U 1.webp",
    "specs": {
      "Тип корпуса": "32TSOP1",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 7,
    "name": "ET1645RU3U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1645RU3U",
    "image": "../assets/images/products/items/ET1645RU3U 1.webp",
    "specs": {
      "Тип корпуса": "44TSOP2",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 8,
    "name": "ET1645RU4",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1645RU4",
    "image": "../assets/images/products/items/ET1645RU4 1.webp",
    "specs": {
      "Тип корпуса": "48TSOP1",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 9,
    "name": "ET1645RU4",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1645RU4",
    "image": "../assets/images/products/items/ET1645RU4 1.webp",
    "specs": {
      "Тип корпуса": "48TSOP2",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 10,
    "name": "ET1645RU7YA",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1645RU7YA",
    "image": "../assets/images/products/items/ET1645RU7YA 1.webp",
    "specs": {
      "Тип корпуса": "100TQFP (20mm x 14mm)",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-60°С…+85°С °C",
      "Напряжение питания": "2,5 В или 3,3 В"
    }
  },
  {
    "id": 11,
    "name": "ET1663RA06V4",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1663RA06V4",
    "image": "../assets/images/products/items/ET1663RA06V4.webp",
    "specs": {
      "Тип корпуса": "48TSOP1",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 12,
    "name": "ET1636RR1",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1636RR1",
    "image": "../assets/images/products/items/ET1636RR1 1.webp",
    "specs": {
      "Тип корпуса": "CQFP44",
      "Тип": "ПАМЯТЬ (MEMORY)",
      "Температурный диапазон": "-55°С…+125°С °C",
      "Напряжение питания": "3,3 В"
    }
  },
  {
    "id": 13,
    "name": "ET5559IN4U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5559IN4U",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "SSOP-24",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "от 4,5 В до 5,5 В"
    }
  },
  {
    "id": 14,
    "name": "ET5559IN10BU",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5559IN10BU",
    "image": "https://ekb-test.ru/upload/uf/20d/ut6ov4lt6vb47k0eowau6ere5pt9k1sx/ET5559IN10BU.png",
    "specs": {
      "Тип корпуса": "SOP8",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "5 В"
    }
  },
  {
    "id": 15,
    "name": "ET5559IN14AU",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5559IN14AU",
    "image": "https://ekb-test.ru/upload/uf/46b/oy7z68a95a497jttny9tk0ut93c5ta1w/ET5559IN14AU.png",
    "specs": {
      "Тип корпуса": "SO-8",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+125°С °C",
      "Напряжение питания": "от 4,5 В до 5,5 В"
    }
  },
  {
    "id": 16,
    "name": "ET5559IN28U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5559IN28U",
    "image": "../assets/images/products/items/5559IN28U.webp",
    "specs": {
      "Тип корпуса": "SOP8",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+125°С °C",
      "Напряжение питания": "от 3,0 до 5,5 В;"
    }
  },
  {
    "id": 17,
    "name": "ET5600VG1U",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5600VG1U",
    "image": "https://ekb-test.ru/upload/uf/4b4/a44f127hodycume6jr1h8wzvhlnkiwib/ET5600VG1U.png",
    "specs": {
      "Тип корпуса": "LQFP64M, LQFP128",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+85°С °C",
      "Напряжение питания": "от 2,7 В до 3,6 В;"
    }
  },
  {
    "id": 18,
    "name": "ET2011VV034",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET2011VV034",
    "image": "https://ekb-test.ru/upload/uf/9a9/ebe4w6j36jv4h48vgbf3iiv3ydzol1wa/ET2011VV034.png",
    "specs": {
      "Тип корпуса": "SO-16, QSOP-16",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-55°С…+125°С °C",
      "Напряжение питания": "от 3 до 5 В"
    }
  },
  {
    "id": 19,
    "name": "ET5572IN1AU",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5572IN1AU",
    "image": "https://ekb-test.ru/upload/uf/8ec/527kh6udm7rz6eovsu3cdcingr6zdr91/ET5572IN1AU.png",
    "specs": {
      "Тип корпуса": "TSSOP48",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+105°С °C"
    }
  },
  {
    "id": 20,
    "name": "ET5572IN2AU",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET5572IN2AU",
    "image": "https://ekb-test.ru/upload/iblock/057/g6nr2g6sktci3qfsk00nbtx8ql3z2d7o/ET5572IN2AU.png",
    "specs": {
      "Тип корпуса": "TSSOP24",
      "Тип": "ИНТЕРФЕЙСЫ (INTERFACES)",
      "Температурный диапазон": "-40°С…+105°С °C"
    }
  },
  {
    "id": 21,
    "name": "ET1986VE-100",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1986VE-100",
    "image": "https://ekb-test.ru/upload/iblock/5fd/gqwgifcm153l80ml1bni4kzgbuojt2ql/ET1986VE_100.png",
    "specs": {
      "Тип корпуса": "LQFP100",
      "Тип": "МИКРОКОНТРОЛЛЕРЫ (MCU)",
      "Температурный диапазон": "-40°С…+105°С °C",
      "Напряжение питания": "2,2 ÷ 3,6 В"
    }
  },
  {
    "id": 22,
    "name": "ET1986VE-64",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET1986VE-64",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "МИКРОКОНТРОЛЛЕРЫ (MCU)",
      "Напряжение питания": "2,2 ÷ 3,6 В"
    }
  },
  {
    "id": 23,
    "name": "ET557BQ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET557BQ",
    "image": "https://ekb-test.ru/upload/iblock/c1a/xf5zeywbgvw5oytaj96hlqza8m70q813/CQFP44.png",
    "specs": {
      "Тип корпуса": "CQFP44",
      "Тип": "ПЛИС И ЗАГРУЗОЧНАЯ ПАМЯТЬ (FPGA AND CONFIGURATION PROM)",
      "Температурный диапазон": "-55…125 °C",
      "Ёмкость": "4"
    }
  },
  {
    "id": 24,
    "name": "ET557BQ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Микросхема ET557BQ_1",
    "image": "https://ekb-test.ru/upload/iblock/c1a/xf5zeywbgvw5oytaj96hlqza8m70q813/CQFP44.png",
    "specs": {
      "Тип корпуса": "CQFP44",
      "Тип": "ПЛИС И ЗАГРУЗОЧНАЯ ПАМЯТЬ (FPGA AND CONFIGURATION PROM)",
      "Температурный диапазон": "-55…125 °C",
      "Ёмкость": "4"
    }
  },
  {
    "id": 25,
    "name": "ET5576BQ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Программируемая логическая ИС (ПЛИС) изготовленная по КМОП-технологии",
    "image": "https://ekb-test.ru/upload/iblock/f5e/pydny6io2937vu53ockd086szqovb3p3/ET5576BQ.png",
    "specs": {
      "Тип": "ПЛИС И ЗАГРУЗОЧНАЯ ПАМЯТЬ (FPGA AND CONFIGURATION PROM)",
      "Температурный диапазон": "-55…125 °C"
    }
  },
  {
    "id": 26,
    "name": "ET5576BQ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы ЭКБ ТЕСТ",
    "description": "Программируемая логическая ИС (ПЛИС) изготовленная по КМОП-технологии",
    "image": "https://ekb-test.ru/upload/iblock/f5e/pydny6io2937vu53ockd086szqovb3p3/ET5576BQ.png",
    "specs": {
      "Тип": "ПЛИС И ЗАГРУЗОЧНАЯ ПАМЯТЬ (FPGA AND CONFIGURATION PROM)",
      "Температурный диапазон": "-55…125 °C"
    }
  },
  {
    "id": 27,
    "name": "1310ПН1У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1310ПН1У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "24-х выводной металлокерамический корпус Н06.24-1В"
    }
  },
  {
    "id": 28,
    "name": "1309ЕН3.3Т",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1309ЕН3.3Т",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "8-выводной металлокерамический корпус 4116.8-3"
    }
  },
  {
    "id": 29,
    "name": "1309ЕС035",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1309ЕС035",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "8-ми выводной металлокерамический корпус Н02.8-1В"
    }
  },
  {
    "id": 30,
    "name": "1309ЕС055",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1309ЕС055",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "8-ми выводной металлокерамический корпус Н02.8-1В"
    }
  },
  {
    "id": 31,
    "name": "1309ЕС065",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1309ЕС065",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "8-ми выводной металлокерамический корпус Н02.8-1В"
    }
  },
  {
    "id": 32,
    "name": "1645РУ1А(Б)У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1645РУ1А(Б)У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "64-х выводной металлокерамический корпус Н18.64-3В"
    }
  },
  {
    "id": 33,
    "name": "1645РУ3А(Б)У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1645РУ3А(Б)У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "64-х выв. металлокерамический корпус Н18.64-3В/МК 5153.64-2"
    }
  },
  {
    "id": 34,
    "name": "1645РУ4АУ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1645РУ4АУ",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "64-выводной металлокерамический корпус 5134.64-6"
    }
  },
  {
    "id": 35,
    "name": "1645РУ6У1",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1645РУ6У1",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "64-в. металлокерам. корп. Н18.64-2В, 64-в. металлокерам. корп. МК 5153.64-2"
    }
  },
  {
    "id": 36,
    "name": "1645РУ7Я",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1645РУ7Я",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "BGA МК 8307.144-АНЗ (16 х 16 мм)"
    }
  },
  {
    "id": 37,
    "name": "1663РА06В4",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1663РА06В4",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "108-х выводной металлокерамический корпус 4226.108-2.03"
    }
  },
  {
    "id": 38,
    "name": "1636РР1А(Б)У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1636РР1А(Б)У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "42-х выводной металлокерамический корпус Н14.42-1В"
    }
  },
  {
    "id": 39,
    "name": "5559ИН4У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5559ИН4У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "28-и выводной металлокерамический корпус Н09.28-1В"
    }
  },
  {
    "id": 40,
    "name": "5559ИН10БУ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5559ИН10БУ",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "8-выводной микро корпус Н02.8-1В, 8-выводной пластиковый корпус SO-8"
    }
  },
  {
    "id": 41,
    "name": "5559ИН14АУ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5559ИН14АУ",
    "image": "https://ekb-test.ru/upload/uf/46b/oy7z68a95a497jttny9tk0ut93c5ta1w/ET5559IN14AU.png",
    "specs": {
      "Тип корпуса": "8-выводной металлокерамический корпус МК 5213.8-1"
    }
  },
  {
    "id": 42,
    "name": "5559ИН28У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5559ИН28У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "Тип корпуса: Н02.8-1В"
    }
  },
  {
    "id": 43,
    "name": "5600ВГ1У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5600ВГ1У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "64-х выводной металлокерамический корпус Н18.64-1В"
    }
  },
  {
    "id": 44,
    "name": "2011BB034",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 2011BB034",
    "image": "https://ekb-test.ru/upload/iblock/546/rv5n5ss5dlqpvkjqxbvg9dyh9dmu7v3h/2011BB034.png",
    "specs": {
      "Тип корпуса": "20-выводной металлокерамический корпус МК 4140.20-1"
    }
  },
  {
    "id": 45,
    "name": "5572ИН1АУ1",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5572ИН1АУ1",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "48-выводной металлокерамический корпус Н16.48-1В"
    }
  },
  {
    "id": 46,
    "name": "5572ИН2АУ",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5572ИН2АУ",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "24-выводной металлокерамический корпус: Н06.24-1В"
    }
  },
  {
    "id": 47,
    "name": "1986ВЕ91Т",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 1986ВЕ91Т",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "132-х выводной металлокерамический корпус 4229.132-3"
    }
  },
  {
    "id": 48,
    "name": "5578РС015",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5578РС015",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "Н09.28-1В"
    }
  },
  {
    "id": 49,
    "name": "5576РС1У",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5576РС1У",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "28-и выводной металлокерамический корпус Н09.28-1В"
    }
  },
  {
    "id": 50,
    "name": "5576ХС4Т",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5576ХС4Т",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "256-выводной металлокерамический корпус 4244.256-3"
    }
  },
  {
    "id": 51,
    "name": "5576ХС1Т",
    "category": "Микросхемы",
    "subcategory": "Микросхемы Миландр",
    "description": "Микросхема 5576ХС1Т",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип корпуса": "240-выводной металлокерамический корпус 4245.240-6"
    }
  },
  {
    "id": 52,
    "name": "Производство разъёмов «под заказ»",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Производство разъёмов «под заказ»",
    "image": "",
    "specs": {}
  },
  {
    "id": 53,
    "name": "ЕТ-СНЦ13",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ13",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 54,
    "name": "ЕТ-СНЦ14",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ14",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 55,
    "name": "ЕТ-СНЦ43",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ43",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 56,
    "name": "ЕТ-ШР",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-ШР",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "1–48",
      "Температурный диапазон": "-60… `+50 °C",
      "Макс. рабочее напряжение": "850 В"
    }
  },
  {
    "id": 57,
    "name": "ЕТ-СШР",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СШР",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "1–48",
      "Температурный диапазон": "-60… `+50 °C",
      "Макс. рабочее напряжение": "850 В"
    }
  },
  {
    "id": 58,
    "name": "ЕТ-2РТТ",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РТТ",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "1–48",
      "Температурный диапазон": "-60… `+150 °C",
      "Макс. рабочее напряжение": "850 В"
    }
  },
  {
    "id": 59,
    "name": "ЕТ-СНЦ13-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ13-2",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "2–47",
      "Температурный диапазон": "-60… `+400 °C",
      "Макс. рабочее напряжение": "350 В"
    }
  },
  {
    "id": 60,
    "name": "ЕТ-СНЦ14-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ14-2",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "2–47",
      "Температурный диапазон": "-60… `+400 °C",
      "Макс. рабочее напряжение": "350 В"
    }
  },
  {
    "id": 61,
    "name": "ЕТ-СНЦ43-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ43-2",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "2–47",
      "Температурный диапазон": "-60… `+400 °C",
      "Макс. рабочее напряжение": "350 В"
    }
  },
  {
    "id": 62,
    "name": "ЕТ-ШР-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-ШР-2",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "5–7",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "300 В"
    }
  },
  {
    "id": 63,
    "name": "ЕТ-СШР-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СШР-2",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "5–7",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "300 В"
    }
  },
  {
    "id": 64,
    "name": "ЕТ-2РТТ-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РТТ-2",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {
      "Количество контактов": "5–7",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "300 В"
    }
  },
  {
    "id": 65,
    "name": "ЕТ-РБМ4",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-РБМ4",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "560 В"
    }
  },
  {
    "id": 66,
    "name": "ЕТ-СНО49",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНО49",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "2–64",
      "Температурный диапазон": "-60… `+155 °C",
      "Макс. рабочее напряжение": "700 В"
    }
  },
  {
    "id": 67,
    "name": "ЕТ-СНО50",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНО50",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "2–64",
      "Температурный диапазон": "-60… `+155 °C",
      "Макс. рабочее напряжение": "700 В"
    }
  },
  {
    "id": 68,
    "name": "ЕТ-СНЦ23",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ23",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 69,
    "name": "ЕТ-СНЦ28",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНЦ28",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 70,
    "name": "ЕТ-РП16",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-РП16",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "700 В"
    }
  },
  {
    "id": 71,
    "name": "ЕТ-2РМ",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РМ",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "560 В"
    }
  },
  {
    "id": 72,
    "name": "ЕТ-2РМД",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РМД",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "560 В"
    }
  },
  {
    "id": 73,
    "name": "ЕТ-РС10",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-РС10",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "2–64",
      "Температурный диапазон": "-60… `+155 °C",
      "Макс. рабочее напряжение": "700 В"
    }
  },
  {
    "id": 74,
    "name": "ЕТ-СНП34",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНП34",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 75,
    "name": "ЕТ-СР",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём ВЧ ЕТ-СР",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Температурный диапазон": "-60… `+155 °C",
      "Макс. рабочее напряжение": "500 В",
      "Сопротивление контактов": "0,01 Ом",
      "Волновое сопротивление": "50,75 Ом"
    }
  },
  {
    "id": 76,
    "name": "ЕТ-2РМ-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РМ-2",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "560 В"
    }
  },
  {
    "id": 77,
    "name": "ЕТ-2РМД-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-2РМД-2",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–55",
      "Температурный диапазон": "-60… `+100 °C",
      "Макс. рабочее напряжение": "560 В"
    }
  },
  {
    "id": 78,
    "name": "ЕТ-СНП34-2",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СНП34-2",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {
      "Количество контактов": "10–102",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "140 В"
    }
  },
  {
    "id": 79,
    "name": "ЕТ-СР",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-СР",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Температурный диапазон": "-60… `+155 °C",
      "Макс. рабочее напряжение": "500 В",
      "Сопротивление контактов": "0,01 Ом",
      "Волновое сопротивление": "50,75 Ом"
    }
  },
  {
    "id": 80,
    "name": "ЕТ-РС19",
    "category": "Разъёмы",
    "subcategory": "Разъёмы ЭКБ ТЕСТ",
    "description": "Разъём многополюсный ЕТ-РС19",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {
      "Количество контактов": "4–50",
      "Температурный диапазон": "-60… `+85 °C",
      "Макс. рабочее напряжение": "200 В"
    }
  },
  {
    "id": 81,
    "name": "СНЦ13",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СНЦ13",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 82,
    "name": "СНЦ14",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СНЦ14",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 83,
    "name": "СНЦ43",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СНЦ43",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 84,
    "name": "ШР",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный ШР",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {}
  },
  {
    "id": 85,
    "name": "СШР",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СШР",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {}
  },
  {
    "id": 86,
    "name": "2РТТ",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный 2РТТ",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {}
  },
  {
    "id": 87,
    "name": "4РТ",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный 4РТ",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {}
  },
  {
    "id": 88,
    "name": "РВН1, РВН2, РВН3",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РВН1, РВН2, РВН3",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 89,
    "name": "РВН1, РВН2, РВН4",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РВН1, РВН2, РВН4",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 90,
    "name": "РВН1, РВН2, РВН5",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РВН1, РВН2, РВН5",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 91,
    "name": "РБМ-4",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РБМ-4",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 92,
    "name": "ШР, СШР, 2РТТ",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный ШР, СШР, 2РТТ",
    "image": "../assets/images/products/items/b2llibz7_ET_2RTT.webp",
    "specs": {}
  },
  {
    "id": 93,
    "name": "РП15",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РП15",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 94,
    "name": "2РМ",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный 2РМ",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 95,
    "name": "2РМД",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный 2РМД",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 96,
    "name": "РС10",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РС10",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 97,
    "name": "СНП34",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СНП34",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 98,
    "name": "СР",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём ВЧ СР",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 99,
    "name": "АЭР-85, АЭР-85М",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный АЭР-85, АЭР-85М",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 99,
    "name": "Реактивный ранец",
    "category": "",
    "subcategory": "",
    "description": "Реактивный ранец: Революция в персональной мобильности и специальных операциях\nРеактивный ранец – инновационное устройство, открывающее новые горизонты в самых разнообразных сферах. От спасения жизней в экстремальных условиях до выполнения секретных миссий и предоставления уникальных развлекательных возможностей – реактивный ранец меняет представление о возможном.\nОсновные области применения:\n•\tАварийно-спасательные операции: Оперативное реагирование при пожарах в высотных зданиях, горно-спасательные работы, экстренное прибытие на места стихийных бедствий.\n•\tВоенные специальные операции: Незаметное проникновение, ведение разведки, быстрая переброска личного состава и преодоление препятствий.\n•\tПромышленные операции: Инспекция и техническое обслуживание объектов энергетики, ветровой энергетики, нефтегазовой отрасли, а также высотного и инфраструктурного оборудования.\n•\tКультурный туризм и развлечения: Захватывающие полеты над живописными ландшафтами, эффектные шоу в тематических парках, экстремальные виды спорта и впечатляющие спецэффекты для кино и телевидения.\n•\tЛичные поездки: Перспективное решение для коротких перемещений на малых высотах, позволяющее избежать транспортных пробок (требует дальнейшего развития технологии).\n",
    "image": "",
    "specs": {}
  },
  {
    "id": 100,
    "name": "ОНЦ-БС, ОНЦ-БМ",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный ОНЦ-БС, ОНЦ-БМ",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 101,
    "name": "СНЦ127",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный СНЦ127",
    "image": "../assets/images/products/items/ET_SNC23 1.webp",
    "specs": {}
  },
  {
    "id": 102,
    "name": "РС(Г)19",
    "category": "Разъёмы",
    "subcategory": "PIN-TO-PIN и функциональные замены разъёмов других производителей",
    "description": "Разъём многополюсный РС(Г)19",
    "image": "../assets/images/products/items/ET_2RMG_D_.webp",
    "specs": {}
  },
  {
    "id": 103,
    "name": "ET-F300C-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F300C-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 104,
    "name": "ET-F300C-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F300C-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 105,
    "name": "ET-F300C-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F300C-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 106,
    "name": "ET-F300C-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F300C-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 107,
    "name": "ET-F300C-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F300C-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 108,
    "name": "ET-F300C-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F300C-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 109,
    "name": "ET-F300C-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F300C-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 110,
    "name": "ET-F300C-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F300C-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 111,
    "name": "ET-F300C-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F300C-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 112,
    "name": "ET-F300H-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F300H-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 113,
    "name": "ET-F300H-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F300H-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 114,
    "name": "ET-F300H-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F300H-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 115,
    "name": "ET-F300H-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F300H-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 116,
    "name": "ET-F300H-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F300H-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 117,
    "name": "ET-F300H-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F300H-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 118,
    "name": "ET-F300H-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F300H-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 119,
    "name": "ET-F300H-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F300H-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 120,
    "name": "ET-F300H-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F300H-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 121,
    "name": "ET-F300M-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F300M-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 122,
    "name": "ET-F300M-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F300M-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 123,
    "name": "ET-F300M-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F300M-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 124,
    "name": "ET-F300M-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F300M-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 125,
    "name": "ET-F300M-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F300M-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 126,
    "name": "ET-F300M-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F300M-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 127,
    "name": "ET-F300M-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F300M-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 128,
    "name": "ET-F300M-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F300M-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 129,
    "name": "ET-F300M-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F300M-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 130,
    "name": "ET-H300C-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H300C-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 131,
    "name": "ET-H300C-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H300C-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 132,
    "name": "ET-H300C-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H300C-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 133,
    "name": "ET-H300C-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H300C-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 134,
    "name": "ET-H300C-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H300C-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 135,
    "name": "ET-H300C-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H300C-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 136,
    "name": "ET-H300C-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H300C-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 137,
    "name": "ET-H300C-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H300C-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 138,
    "name": "ET-H300C-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H300C-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 139,
    "name": "ET-H300H-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H300H-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 140,
    "name": "ET-H300H-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H300H-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 141,
    "name": "ET-H300H-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H300H-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 142,
    "name": "ET-H300H-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H300H-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 143,
    "name": "ET-H300H-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H300H-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 144,
    "name": "ET-H300H-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H300H-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 145,
    "name": "ET-H300H-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H300H-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 146,
    "name": "ET-H300H-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H300H-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 147,
    "name": "ET-H300H-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H300H-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 148,
    "name": "ET-H300M-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H300M-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 149,
    "name": "ET-H300M-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H300M-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 150,
    "name": "ET-H300M-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H300M-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 151,
    "name": "ET-H300M-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H300M-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 152,
    "name": "ET-H300M-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H300M-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 153,
    "name": "ET-H300M-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H300M-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 154,
    "name": "ET-H300M-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H300M-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 155,
    "name": "ET-H300M-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H300M-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 156,
    "name": "ET-H300M-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H300M-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 157,
    "name": "ET-Q300C-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q300C-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 158,
    "name": "ET-Q300C-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q300C-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 159,
    "name": "ET-Q300C-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q300C-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 160,
    "name": "ET-Q300C-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q300C-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 161,
    "name": "ET-Q300C-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q300C-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 162,
    "name": "ET-Q300C-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q300C-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 163,
    "name": "ET-Q300C-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q300C-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 164,
    "name": "ET-Q300C-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q300C-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 165,
    "name": "ET-Q300C-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q300C-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 166,
    "name": "ET-Q300H-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q300H-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 167,
    "name": "ET-Q300H-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q300H-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 168,
    "name": "ET-Q300H-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q300H-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 169,
    "name": "ET-Q300H-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q300H-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 170,
    "name": "ET-Q300H-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q300H-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 171,
    "name": "ET-Q300H-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q300H-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 172,
    "name": "ET-Q300H-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q300H-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 173,
    "name": "ET-Q300H-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q300H-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 174,
    "name": "ET-Q300H-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q300H-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 175,
    "name": "ET-Q300M-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q300M-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 176,
    "name": "ET-Q300M-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q300M-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 177,
    "name": "ET-Q300M-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q300M-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 178,
    "name": "ET-Q300M-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q300M-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 179,
    "name": "ET-Q300M-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q300M-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 180,
    "name": "ET-Q300M-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q300M-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 181,
    "name": "ET-Q300M-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q300M-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 182,
    "name": "ET-Q300M-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q300M-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 183,
    "name": "ET-Q300M-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q300M-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q300",
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 184,
    "name": "ET-F375C-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F375C-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 185,
    "name": "ET-F375C-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F375C-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 186,
    "name": "ET-F375C-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F375C-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 187,
    "name": "ET-F375C-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F375C-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 188,
    "name": "ET-F375C-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F375C-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 189,
    "name": "ET-F375C-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F375C-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 190,
    "name": "ET-F375C-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F375C-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 191,
    "name": "ET-F375C-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F375C-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 192,
    "name": "ET-F375C-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F375C-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 193,
    "name": "ET-F375H-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F375H-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 194,
    "name": "ET-F375H-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F375H-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 195,
    "name": "ET-F375H-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F375H-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 196,
    "name": "ET-F375H-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F375H-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 197,
    "name": "ET-F375H-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F375H-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 198,
    "name": "ET-F375H-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F375H-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 199,
    "name": "ET-F375H-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F375H-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 200,
    "name": "ET-F375H-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F375H-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 201,
    "name": "ET-F375H-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F375H-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 202,
    "name": "ET-F375M-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F375M-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 203,
    "name": "ET-F375M-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 400 Вт ET-F375M-05VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 204,
    "name": "ET-F375M-08VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F375M-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 205,
    "name": "ET-F375M-12VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 600 Вт ET-F375M-12VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 206,
    "name": "ET-F375M-15VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 600 Вт ET-F375M-15VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 207,
    "name": "ET-F375M-24VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 600 Вт ET-F375M-24VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 208,
    "name": "ET-F375M-28VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 600 Вт ET-F375M-28VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 209,
    "name": "ET-F375M-36VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 600 Вт ET-F375M-36VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 210,
    "name": "ET-F375M-48VP600R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 600 Вт ET-F375M-48VP600R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "600 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 211,
    "name": "ET-H375C-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H375C-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 212,
    "name": "ET-H375C-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H375C-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 213,
    "name": "ET-H375C-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H375C-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 214,
    "name": "ET-H375C-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H375C-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 215,
    "name": "ET-H375C-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H375C-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 216,
    "name": "ET-H375C-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H375C-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 217,
    "name": "ET-H375C-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H375C-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 218,
    "name": "ET-H375C-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H375C-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 219,
    "name": "ET-H375C-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H375C-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 220,
    "name": "ET-H375H-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H375H-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 221,
    "name": "ET-H375H-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H375H-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 222,
    "name": "ET-H375H-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H375H-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 223,
    "name": "ET-H375H-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H375H-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 224,
    "name": "ET-H375H-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H375H-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 225,
    "name": "ET-H375H-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H375H-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 226,
    "name": "ET-H375H-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H375H-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 227,
    "name": "ET-H375H-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H375H-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 228,
    "name": "ET-H375H-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H375H-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 229,
    "name": "ET-H375M-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H375M-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 230,
    "name": "ET-H375M-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H375M-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 231,
    "name": "ET-H375M-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H375M-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 232,
    "name": "ET-H375M-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H375M-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 233,
    "name": "ET-H375M-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H375M-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 234,
    "name": "ET-H375M-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H375M-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 235,
    "name": "ET-H375M-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H375M-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 236,
    "name": "ET-H375M-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H375M-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 237,
    "name": "ET-H375M-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H375M-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 238,
    "name": "ET-Q375C-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q375C-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 239,
    "name": "ET-Q375C-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q375C-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 240,
    "name": "ET-Q375C-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q375C-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 241,
    "name": "ET-Q375C-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q375C-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 242,
    "name": "ET-Q375C-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q375C-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 243,
    "name": "ET-Q375C-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q375C-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 244,
    "name": "ET-Q375C-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q375C-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 245,
    "name": "ET-Q375C-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q375C-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 246,
    "name": "ET-Q375C-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q375C-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 247,
    "name": "ET-Q375H-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q375H-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 248,
    "name": "ET-Q375H-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q375H-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 249,
    "name": "ET-Q375H-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q375H-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 250,
    "name": "ET-Q375H-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q375H-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 251,
    "name": "ET-Q375H-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q375H-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 252,
    "name": "ET-Q375H-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q375H-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 253,
    "name": "ET-Q375H-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q375H-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 254,
    "name": "ET-Q375H-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q375H-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 255,
    "name": "ET-Q375H-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q375H-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 256,
    "name": "ET-Q375M-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-Q375M-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 257,
    "name": "ET-Q375M-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-Q375M-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 258,
    "name": "ET-Q375M-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-Q375M-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 259,
    "name": "ET-Q375M-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-Q375M-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 260,
    "name": "ET-Q375M-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-Q375M-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 261,
    "name": "ET-Q375M-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-Q375M-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,25 А"
    }
  },
  {
    "id": 262,
    "name": "ET-Q375M-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-Q375M-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,36 А"
    }
  },
  {
    "id": 263,
    "name": "ET-Q375M-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-Q375M-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "4,16 А"
    }
  },
  {
    "id": 264,
    "name": "ET-Q375M-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-Q375M-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q375",
      "Входное напряжение": "375 (250..425) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "3,13 А"
    }
  },
  {
    "id": 265,
    "name": "ET-F24C-05VP300R30",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F24C-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 266,
    "name": "ET-F24C-05VP300R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 300 Вт ET-F24C-05VP300R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "300 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "60 А"
    }
  },
  {
    "id": 267,
    "name": "ET-F24C-05VP300R24",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F24C-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 268,
    "name": "ET-F24C-05VP300R25",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 500 Вт ET-F24C-12VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 269,
    "name": "ET-F24C-05VP300R26",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 500 Вт ET-F24C-15VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 270,
    "name": "ET-F24C-05VP300R27",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 500 Вт ET-F24C-24VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 271,
    "name": "ET-F24C-05VP300R28",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 500 Вт ET-F24C-28VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 272,
    "name": "ET-F24C-05VP300R29",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 500 Вт ET-F24C-36VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 273,
    "name": "ET-F24C-05VP300R31",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 500 Вт ET-F24C-48VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 274,
    "name": "ET-F24C-05VP300R46",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F24H-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 275,
    "name": "ET-F24C-05VP300R32",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 300 Вт ET-F24H-05VP300R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "300 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "60 А"
    }
  },
  {
    "id": 276,
    "name": "ET-F24C-05VP300R35",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F24H-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 277,
    "name": "ET-F24C-05VP300R37",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 500 Вт ET-F24H-12VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 278,
    "name": "ET-F24C-05VP300R39",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 500 Вт ET-F24H-15VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 279,
    "name": "ET-F24C-05VP300R41",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 500 Вт ET-F24H-24VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 280,
    "name": "ET-F24C-05VP300R43",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 500 Вт ET-F24H-28VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 281,
    "name": "ET-F24C-05VP300R45",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 500 Вт ET-F24H-36VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 282,
    "name": "ET-F24C-05VP300R49",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 500 Вт ET-F24H-48VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 283,
    "name": "ET-F24M-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 264 Вт ET-F24M-3V3P264R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "264 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "80 А"
    }
  },
  {
    "id": 284,
    "name": "ET-F24C-05VP300R50",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 300 Вт ET-F24M-05VP300R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "300 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "60 А"
    }
  },
  {
    "id": 285,
    "name": "ET-F24C-05VP300R53",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 400 Вт ET-F24M-08VP400R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "400 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 286,
    "name": "ET-F24M-12VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 500 Вт ET-F24M-12VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "50 А"
    }
  },
  {
    "id": 287,
    "name": "ET-F24M-15VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 500 Вт ET-F24M-15VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 288,
    "name": "ET-F24M-24VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 500 Вт ET-F24M-24VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 289,
    "name": "ET-F24M-28VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 500 Вт ET-F24M-28VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "21,5 А"
    }
  },
  {
    "id": 290,
    "name": "ET-F24M-36VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 500 Вт ET-F24M-36VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 291,
    "name": "ET-F24M-48VP500R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 500 Вт ET-F24M-48VP500R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "500 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 292,
    "name": "ET-H24C-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H24C-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 293,
    "name": "ET-H24C-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H24C-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 294,
    "name": "ET-H24C-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H24C-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 295,
    "name": "ET-H24C-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H24C-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 296,
    "name": "ET-H24C-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H24C-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 297,
    "name": "ET-H24C-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H24C-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 298,
    "name": "ET-H24C-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H24C-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 299,
    "name": "ET-H24C-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H24C-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 300,
    "name": "ET-H24C-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H24C-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 301,
    "name": "ET-H24H-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H24H-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 302,
    "name": "ET-H24H-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H24H-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 303,
    "name": "ET-H24H-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H24H-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 304,
    "name": "ET-H24H-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H24H-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 305,
    "name": "ET-H24H-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H24H-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 306,
    "name": "ET-H24H-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H24H-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 307,
    "name": "ET-H24H-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H24H-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 308,
    "name": "ET-H24H-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H24H-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 309,
    "name": "ET-H24H-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H24H-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 310,
    "name": "ET-H24M-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-H24M-3V3P150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 311,
    "name": "ET-H24M-05VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 200 Вт ET-H24M-05VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 312,
    "name": "ET-H24M-08VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 200 Вт ET-H24M-08VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "25 А"
    }
  },
  {
    "id": 313,
    "name": "ET-H24M-12VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 250 Вт ET-H24M-12VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 314,
    "name": "ET-H24M-15VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 250 Вт ET-H24M-15VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 315,
    "name": "ET-H24M-24VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 250 Вт ET-H24M-24VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 316,
    "name": "ET-H24M-28VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 250 Вт ET-H24M-28VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 317,
    "name": "ET-H24M-36VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 250 Вт ET-H24M-36VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 318,
    "name": "ET-H24M-48VP250R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 250 Вт ET-H24M-48VP250R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "250 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 319,
    "name": "ET-Q24C-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-Q24C-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 320,
    "name": "ET-Q24C-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 100 Вт ET-Q24C-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 321,
    "name": "ET-Q24C-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 100 Вт ET-Q24C-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 322,
    "name": "ET-Q24C-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 150 Вт ET-Q24C-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 323,
    "name": "ET-Q24C-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-Q24C-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 324,
    "name": "ET-Q24C-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-Q24C-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,3 А"
    }
  },
  {
    "id": 325,
    "name": "ET-Q24C-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-Q24C-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,4 А"
    }
  },
  {
    "id": 326,
    "name": "ET-Q24C-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-Q24C-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 327,
    "name": "ET-Q24C-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-Q24C-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "3,1 А"
    }
  },
  {
    "id": 328,
    "name": "ET-Q24H-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-Q24H-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 329,
    "name": "ET-Q24H-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 100 Вт ET-Q24H-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 330,
    "name": "ET-Q24H-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 100 Вт ET-Q24H-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 331,
    "name": "ET-Q24H-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 150 Вт ET-Q24H-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 332,
    "name": "ET-Q24H-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-Q24H-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 333,
    "name": "ET-Q24H-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-Q24H-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,3 А"
    }
  },
  {
    "id": 334,
    "name": "ET-Q24H-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-Q24H-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,4 А"
    }
  },
  {
    "id": 335,
    "name": "ET-Q24H-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-Q24H-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 336,
    "name": "ET-Q24H-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-Q24H-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "3,1 А"
    }
  },
  {
    "id": 337,
    "name": "ET-Q24M-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-Q24M-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "22,7 А"
    }
  },
  {
    "id": 338,
    "name": "ET-Q24M-05VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 100 Вт ET-Q24M-05VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20 А"
    }
  },
  {
    "id": 339,
    "name": "ET-Q24M-08VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 8 В / 100 Вт ET-Q24M-08VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "8 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 340,
    "name": "ET-Q24M-12VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 150 Вт ET-Q24M-12VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "12,5 А"
    }
  },
  {
    "id": 341,
    "name": "ET-Q24M-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-Q24M-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 342,
    "name": "ET-Q24M-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-Q24M-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,3 А"
    }
  },
  {
    "id": 343,
    "name": "ET-Q24M-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-Q24M-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,4 А"
    }
  },
  {
    "id": 344,
    "name": "ET-Q24M-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-Q24M-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 345,
    "name": "ET-Q24M-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-Q24M-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q24",
      "Входное напряжение": "24 (18..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "3,1 А"
    }
  },
  {
    "id": 346,
    "name": "ET-F28C-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-F28C-3V3P150R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "45,5 А"
    }
  },
  {
    "id": 347,
    "name": "ET-F28C-05VP175R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 175 Вт ET-F28C-05VP175R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "175 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "35 А"
    }
  },
  {
    "id": 348,
    "name": "ET-F28C-12VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 200 Вт ET-F28C-12VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 349,
    "name": "ET-F28C-15VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 200 Вт ET-F28C-15VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "13,3 А"
    }
  },
  {
    "id": 350,
    "name": "ET-F28C-24VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 200 Вт ET-F28C-24VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 351,
    "name": "ET-F28C-28VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 200 Вт ET-F28C-28VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "7,1 А"
    }
  },
  {
    "id": 352,
    "name": "ET-F28C-36VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 200 Вт ET-F28C-36VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,6 А"
    }
  },
  {
    "id": 353,
    "name": "ET-F28C-48VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 200 Вт ET-F28C-48VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 354,
    "name": "ET-F28H-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-F28H-3V3P150R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "45,5 А"
    }
  },
  {
    "id": 355,
    "name": "ET-F28H-05VP175R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 175 Вт ET-F28H-05VP175R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "175 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "35 А"
    }
  },
  {
    "id": 356,
    "name": "ET-F28H-12VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 200 Вт ET-F28H-12VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 357,
    "name": "ET-F28H-15VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 200 Вт ET-F28H-15VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "13,3 А"
    }
  },
  {
    "id": 358,
    "name": "ET-F28H-24VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 200 Вт ET-F28H-24VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 359,
    "name": "ET-F28H-28VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 200 Вт ET-F28H-28VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "7,1 А"
    }
  },
  {
    "id": 360,
    "name": "ET-F28H-36VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 200 Вт ET-F28H-36VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,6 А"
    }
  },
  {
    "id": 361,
    "name": "ET-F28H-48VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 200 Вт ET-F28H-48VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 362,
    "name": "ET-F28M-3V3P150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 150 Вт ET-F28M-3V3P150R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "45,5 А"
    }
  },
  {
    "id": 363,
    "name": "ET-F28M-05VP175R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 175 Вт ET-F28M-05VP175R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "175 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "35 А"
    }
  },
  {
    "id": 364,
    "name": "ET-F28M-12VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 200 Вт ET-F28M-12VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 365,
    "name": "ET-F28M-15VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 200 Вт ET-F28M-15VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "13,3 А"
    }
  },
  {
    "id": 366,
    "name": "ET-F28M-24VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 200 Вт ET-F28M-24VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 367,
    "name": "ET-F28M-28VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 200 Вт ET-F28M-28VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "7,1 А"
    }
  },
  {
    "id": 368,
    "name": "ET-F28M-36VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 200 Вт ET-F28M-36VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,6 А"
    }
  },
  {
    "id": 369,
    "name": "ET-F28M-48VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 200 Вт ET-F28M-48VP200R23",
    "image": "../assets/images/products/items/ebbfosux_ET_F 1.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-F28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "200 Вт",
      "Тип корпуса": "full brick",
      "Размеры": "117 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 370,
    "name": "ET-H28C-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-H28C-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 371,
    "name": "ET-H28C-05VP075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 75 Вт ET-H28C-05VP075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 372,
    "name": "ET-H28C-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-H28C-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 373,
    "name": "ET-H28C-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-H28C-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 374,
    "name": "ET-H28C-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-H28C-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 375,
    "name": "ET-H28C-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-H28C-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 376,
    "name": "ET-H28C-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-H28C-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 377,
    "name": "ET-H28C-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-H28C-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 378,
    "name": "ET-H28H-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-H28H-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 379,
    "name": "ET-H28H-05VP075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 75 Вт ET-H28H-05VP075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 380,
    "name": "ET-H28H-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-H28H-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 381,
    "name": "ET-H28H-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-H28H-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 382,
    "name": "ET-H28H-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-H28H-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 383,
    "name": "ET-H28H-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-H28H-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 384,
    "name": "ET-H28H-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-H28H-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 385,
    "name": "ET-H28H-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-H28H-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 386,
    "name": "ET-H28M-3V3P075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 75 Вт ET-H28M-3V3P075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "45,4 А"
    }
  },
  {
    "id": 387,
    "name": "ET-H28M-05VP075R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 75 Вт ET-H28M-05VP075R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "75 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "40 А"
    }
  },
  {
    "id": 388,
    "name": "ET-H28M-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-H28M-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "20,8 А"
    }
  },
  {
    "id": 389,
    "name": "ET-H28M-15VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 150 Вт ET-H28M-15VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "16,7 А"
    }
  },
  {
    "id": 390,
    "name": "ET-H28M-24VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 150 Вт ET-H28M-24VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10,4 А"
    }
  },
  {
    "id": 391,
    "name": "ET-H28M-28VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 150 Вт ET-H28M-28VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,9 А"
    }
  },
  {
    "id": 392,
    "name": "ET-H28M-36VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 150 Вт ET-H28M-36VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,9 А"
    }
  },
  {
    "id": 393,
    "name": "ET-H28M-48VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 150 Вт ET-H28M-48VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-H28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "150 Вт",
      "Тип корпуса": "1/2 brick",
      "Размеры": "57,9 × 55,9 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "5,2 А"
    }
  },
  {
    "id": 394,
    "name": "ET-Q28C-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 50 Вт ET-Q28C-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "15,2 А"
    }
  },
  {
    "id": 395,
    "name": "ET-Q28C-05VP050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 50 Вт ET-Q28C-05VP050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 396,
    "name": "ET-Q28C-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-Q28C-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 397,
    "name": "ET-Q28C-15VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 100 Вт ET-Q28C-15VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "6,7 А"
    }
  },
  {
    "id": 398,
    "name": "ET-Q28C-24VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 100 Вт ET-Q28C-24VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 399,
    "name": "ET-Q28C-28VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 100 Вт ET-Q28C-28VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "3,6 А"
    }
  },
  {
    "id": 400,
    "name": "ET-Q28C-36VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 100 Вт ET-Q28C-36VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "2,8 А"
    }
  },
  {
    "id": 401,
    "name": "ET-Q28C-48VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 100 Вт ET-Q28C-48VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "C (-20..100)",
      "Макс. ток": "2,1 А"
    }
  },
  {
    "id": 402,
    "name": "ET-Q28H-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 50 Вт ET-Q28H-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "15,2 А"
    }
  },
  {
    "id": 403,
    "name": "ET-Q28H-05VP050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 50 Вт ET-Q28H-05VP050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 404,
    "name": "ET-Q28H-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-Q28H-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 405,
    "name": "ET-Q28H-15VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 100 Вт ET-Q28H-15VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "6,7 А"
    }
  },
  {
    "id": 406,
    "name": "ET-Q28H-24VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 100 Вт ET-Q28H-24VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 407,
    "name": "ET-Q28H-28VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 100 Вт ET-Q28H-28VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "3,6 А"
    }
  },
  {
    "id": 408,
    "name": "ET-Q28H-36VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 100 Вт ET-Q28H-36VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "2,8 А"
    }
  },
  {
    "id": 409,
    "name": "ET-Q28H-48VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 100 Вт ET-Q28H-48VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "H (-40..100)",
      "Макс. ток": "2,1 А"
    }
  },
  {
    "id": 410,
    "name": "ET-Q28M-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 3,3 В / 50 Вт ET-Q28M-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "15,2 А"
    }
  },
  {
    "id": 411,
    "name": "ET-Q28M-05VP050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 5 В / 50 Вт ET-Q28M-05VP050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Мощность": "50 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "10 А"
    }
  },
  {
    "id": 412,
    "name": "ET-Q28M-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 12 В / 100 Вт ET-Q28M-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "8,3 А"
    }
  },
  {
    "id": 413,
    "name": "ET-Q28M-15VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 15 В / 100 Вт ET-Q28M-15VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "6,7 А"
    }
  },
  {
    "id": 414,
    "name": "ET-Q28M-24VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 24 В / 100 Вт ET-Q28M-24VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "4,2 А"
    }
  },
  {
    "id": 415,
    "name": "ET-Q28M-28VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 28 В / 100 Вт ET-Q28M-28VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "3,6 А"
    }
  },
  {
    "id": 416,
    "name": "ET-Q28M-36VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 36 В / 100 Вт ET-Q28M-36VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "2,8 А"
    }
  },
  {
    "id": 417,
    "name": "ET-Q28M-48VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш 48 В / 100 Вт ET-Q28M-48VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC",
      "Серия": "ET-Q28",
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Мощность": "100 Вт",
      "Тип корпуса": "1/4 brick",
      "Размеры": "57,9 × 36,8 × 12,7 мм",
      "Температурный диапазон": "M (-55..100)",
      "Макс. ток": "2,1 А"
    }
  },
  {
    "id": 418,
    "name": "V300 maxi",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V300 maxi",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V300 maxi"
    }
  },
  {
    "id": 419,
    "name": "V300 mini",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V300 mini",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V300 mini"
    }
  },
  {
    "id": 420,
    "name": "V300 micro",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V300 micro",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V300 micro"
    }
  },
  {
    "id": 421,
    "name": "V375 maxi",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V375 maxi",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V375 maxi"
    }
  },
  {
    "id": 422,
    "name": "V375 mini",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V375 mini",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V375 mini"
    }
  },
  {
    "id": 423,
    "name": "V375 micro",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V375 micro",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V375 micro"
    }
  },
  {
    "id": 424,
    "name": "V24 maxi",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V24 maxi",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V24 maxi"
    }
  },
  {
    "id": 425,
    "name": "V24 mini",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V24 mini",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V24 mini"
    }
  },
  {
    "id": 426,
    "name": "V24 micro",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V24 micro",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V24 micro"
    }
  },
  {
    "id": 427,
    "name": "V28 maxi",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V28 maxi",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V28 maxi"
    }
  },
  {
    "id": 428,
    "name": "V28 mini",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V28 mini",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V28 mini"
    }
  },
  {
    "id": 429,
    "name": "V28 micro",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, серия V28 micro",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Серия": "V28 micro"
    }
  },
  {
    "id": 430,
    "name": "V24A3V3H264BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A3V3H264BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 431,
    "name": "V24A5H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A5H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 432,
    "name": "V24A8H300BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A8H300BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 433,
    "name": "V24A12H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A12H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 434,
    "name": "V24A15H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A15H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 435,
    "name": "V24A24H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A24H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 436,
    "name": "V24A28H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A28H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 437,
    "name": "V24A36H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A36H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 438,
    "name": "V24A48H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A48H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 439,
    "name": "V24A3V3M264BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A3V3M264BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 440,
    "name": "V24A5M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A5M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 441,
    "name": "V24A8M300BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A8M300BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 442,
    "name": "V24A12M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A12M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 443,
    "name": "V24A15M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A15M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 444,
    "name": "V24A24M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A24M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 445,
    "name": "V24A28M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A28M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 446,
    "name": "V24A36M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A36M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 447,
    "name": "V24A48M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24A48M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 448,
    "name": "V24B3V3H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B3V3H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 449,
    "name": "V24B5H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B5H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 450,
    "name": "V24B8H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B8H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 451,
    "name": "V24B12H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B12H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 452,
    "name": "V24B15H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B15H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 453,
    "name": "V24B24H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B24H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 454,
    "name": "V24B28H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B28H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 455,
    "name": "V24B36H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B36H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 456,
    "name": "V24B48H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B48H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 457,
    "name": "V24B3V3M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B3V3M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 458,
    "name": "V24B5M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B5M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 459,
    "name": "V24B8M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B8M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 460,
    "name": "V24B12M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B12M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 461,
    "name": "V24B15M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B15M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 462,
    "name": "V24B24M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B24M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 463,
    "name": "V24B28M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B28M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 464,
    "name": "V24B36M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B36M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 465,
    "name": "V24B48M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24B48M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 466,
    "name": "V24C3V3H50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C3V3H50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 467,
    "name": "V24C5H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C5H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 468,
    "name": "V24C8H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C8H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 469,
    "name": "V24C12H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C12H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 470,
    "name": "V24C15H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C15H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 471,
    "name": "V24C24H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C24H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 472,
    "name": "V24C28H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C28H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 473,
    "name": "V24C36H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C36H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 474,
    "name": "V24C48H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C48H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 475,
    "name": "V24C3V3M50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C3V3M50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 476,
    "name": "V24C5M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C5M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 477,
    "name": "V24C8M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C8M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 478,
    "name": "V24C12M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C12M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 479,
    "name": "V24C15M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C15M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 480,
    "name": "V24C24M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C24M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 481,
    "name": "V24C28M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C28M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 482,
    "name": "V24C36M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C36M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 483,
    "name": "V24C48M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V24C48M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "24 (18..36В) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 484,
    "name": "V300A3V3H264BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A3V3H264BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 485,
    "name": "V300A5H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A5H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 486,
    "name": "V300A8H400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A8H400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 487,
    "name": "V300A12H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A12H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 488,
    "name": "V300A15H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A15H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 489,
    "name": "V300A24H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A24H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 490,
    "name": "V300A28H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A28H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 491,
    "name": "V300A36H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A36H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 492,
    "name": "V300A48H500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A48H500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 493,
    "name": "V300A3V3M264BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A3V3M264BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 494,
    "name": "V300A5M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A5M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 495,
    "name": "V300A8M400BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A8M400BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 496,
    "name": "V300A12M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A12M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 497,
    "name": "V300A15M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A15M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 498,
    "name": "V300A24M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A24M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 499,
    "name": "V300A28M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A28M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 500,
    "name": "V300A36M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A36M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 501,
    "name": "V300A48M500BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300A48M500BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 502,
    "name": "V300B3V3H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B3V3H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 503,
    "name": "V300B5H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B5H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 504,
    "name": "V300B8H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B8H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 505,
    "name": "V300B12H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B12H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 506,
    "name": "V300B15H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B15H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 507,
    "name": "V300B24H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B24H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 508,
    "name": "V300B28H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B28H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 509,
    "name": "V300B36H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B36H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 510,
    "name": "V300B48H250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B48H250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 511,
    "name": "V300B3V3M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B3V3M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 512,
    "name": "V300B5M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B5M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 513,
    "name": "V300B8M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B8M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 514,
    "name": "V300B12M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B12M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 515,
    "name": "V300B15M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B15M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 516,
    "name": "V300B24M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B24M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 517,
    "name": "V300B28M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B28M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 518,
    "name": "V300B36M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B36M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 519,
    "name": "V300B48M250BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300B48M250BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 520,
    "name": "V300C3V3H50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C3V3H50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 521,
    "name": "V300C5H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C5H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 522,
    "name": "V300C8H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C8H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 523,
    "name": "V300C12H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C12H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 524,
    "name": "V300C15H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C15H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 525,
    "name": "V300C24H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C24H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 526,
    "name": "V300C28H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C28H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 527,
    "name": "V300C36H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C36H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 528,
    "name": "V300C48H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C48H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…108"
    }
  },
  {
    "id": 529,
    "name": "V300C3V3M50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C3V3M50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 530,
    "name": "V300C5M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C5M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 531,
    "name": "V300C8M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C8M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "8 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 532,
    "name": "V300C12M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C12M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 533,
    "name": "V300C15M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C15M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 534,
    "name": "V300C24M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C24M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 535,
    "name": "V300C28M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C28M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 536,
    "name": "V300C36M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C36M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 537,
    "name": "V300C48M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V300C48M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "300 (180..375) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…108"
    }
  },
  {
    "id": 538,
    "name": "V28A3V3H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A3V3H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 539,
    "name": "V28A5H175BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A5H175BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 540,
    "name": "V28A12H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A12H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 541,
    "name": "V28A15H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A15H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 542,
    "name": "V28A24H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A24H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 543,
    "name": "V28A28H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A28H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 544,
    "name": "V28A36H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A36H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 545,
    "name": "V28A48H200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A48H200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 546,
    "name": "V28A3V3M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A3V3M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 547,
    "name": "V28A5M175BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A5M175BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 548,
    "name": "V28A12M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A12M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 549,
    "name": "V28A15M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A15M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 550,
    "name": "V28A24M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A24M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 551,
    "name": "V28A28M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A28M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 552,
    "name": "V28A36M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A36M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 553,
    "name": "V28A48M200BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28A48M200BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "Full brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 554,
    "name": "V28B3V3H75BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B3V3H75BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 555,
    "name": "V28B5H75BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B5H75BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 556,
    "name": "V28B12H125BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B12H125BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 557,
    "name": "V28B15H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B15H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 558,
    "name": "V28B24H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B24H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 559,
    "name": "V28B28H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B28H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 560,
    "name": "V28B36H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B36H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 561,
    "name": "V28B48H150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B48H150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 562,
    "name": "V28B3V3M75BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B3V3M75BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 563,
    "name": "V28B5M75BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B5M75BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 564,
    "name": "V28B12M125BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B12M125BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 565,
    "name": "V28B15M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B15M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 566,
    "name": "V28B24M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B24M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 567,
    "name": "V28B28M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B28M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 568,
    "name": "V28B36M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B36M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 569,
    "name": "V28B48M150BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28B48M150BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/2 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 570,
    "name": "V28C3V3H50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C3V3H50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…100"
    }
  },
  {
    "id": 571,
    "name": "V28C5H50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C5H50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…101"
    }
  },
  {
    "id": 572,
    "name": "V28C12H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C12H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…102"
    }
  },
  {
    "id": 573,
    "name": "V28C15H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C15H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…103"
    }
  },
  {
    "id": 574,
    "name": "V28C24H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C24H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…104"
    }
  },
  {
    "id": 575,
    "name": "V28C28H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C28H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…105"
    }
  },
  {
    "id": 576,
    "name": "V28C36H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C36H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…106"
    }
  },
  {
    "id": 577,
    "name": "V28C48H100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C48H100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-40…107"
    }
  },
  {
    "id": 578,
    "name": "V28C3V3M50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C3V3M50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "3,3 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…100"
    }
  },
  {
    "id": 579,
    "name": "V28C5M50BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C5M50BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "5 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…101"
    }
  },
  {
    "id": 580,
    "name": "V28C12M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C12M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "12 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…102"
    }
  },
  {
    "id": 581,
    "name": "V28C15M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C15M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "15 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…103"
    }
  },
  {
    "id": 582,
    "name": "V28C24M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C24M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "24 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…104"
    }
  },
  {
    "id": 583,
    "name": "V28C28M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C28M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "28 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…105"
    }
  },
  {
    "id": 584,
    "name": "V28C36M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C36M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "36 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…106"
    }
  },
  {
    "id": 585,
    "name": "V28C48M100BL",
    "category": "Преобразователи напряжения",
    "subcategory": "Полные pin-to-pin замены преобразователей Vicor",
    "description": "Vicor, V28C48M100BL",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Входное напряжение": "28 (9..36) В",
      "Выходное напряжение": "48 В",
      "Тип корпуса": "1/4 brick",
      "Температурный диапазон": "-55…107"
    }
  },
  {
    "id": 586,
    "name": "ET-F24H-3V3P264R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-3V3P264R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 587,
    "name": "ET-F24H-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-05VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 588,
    "name": "ET-F24H-08VP300R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-08VP300R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 589,
    "name": "ET-F24H-12VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-12VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 590,
    "name": "ET-F24H-15VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-15VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 591,
    "name": "ET-F24H-24VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-24VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 592,
    "name": "ET-F24H-28VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-28VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 593,
    "name": "ET-F24H-36VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-36VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 594,
    "name": "ET-F24H-48VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24H-48VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 595,
    "name": "ET-F24M-05VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-05VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 596,
    "name": "ET-F24M-08VP300R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-08VP300R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 597,
    "name": "ET-F24M-12VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-12VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 598,
    "name": "ET-F24M-15VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-15VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 599,
    "name": "ET-F24M-24VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-24VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 600,
    "name": "ET-F24M-28VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-28VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 601,
    "name": "ET-F24M-36VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-36VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 602,
    "name": "ET-F24M-48VP400R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-F24M-48VP400R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 603,
    "name": "ET-H24H-05VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-05VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 604,
    "name": "ET-H24H-12VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-12VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 605,
    "name": "ET-H24H-15VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-15VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 606,
    "name": "ET-H24H-24VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-24VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 607,
    "name": "ET-H24H-28VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-28VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 608,
    "name": "ET-H24H-36VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-36VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 609,
    "name": "ET-H24H-3V3P100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-3V3P100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 610,
    "name": "ET-H24H-48VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24H-48VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 611,
    "name": "ET-H24M-05VP150R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-05VP150R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 612,
    "name": "ET-H24M-12VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-12VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 613,
    "name": "ET-H24M-15VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-15VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 614,
    "name": "ET-H24M-24VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-24VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 615,
    "name": "ET-H24M-28VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-28VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 616,
    "name": "ET-H24M-36VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-36VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 617,
    "name": "ET-H24M-3V3P100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-3V3P100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 618,
    "name": "ET-H24M-48VP200R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H24M-48VP200R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 619,
    "name": "ET-H28H-05VP75R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28H-05VP75R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 620,
    "name": "ET-H28H-12VP125R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28H-12VP125R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 621,
    "name": "ET-H28H-3V3P75R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28H-3V3P75R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 622,
    "name": "ET-H28M-05VP75R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28M-05VP75R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 623,
    "name": "ET-H28M-12VP125R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28M-12VP125R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 624,
    "name": "ET-H28M-3V3P75R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-H28M-3V3P75R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 625,
    "name": "ET-Q24H-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 626,
    "name": "ET-Q24H-15VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-15VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 627,
    "name": "ET-Q24H-24VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-24VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 628,
    "name": "ET-Q24H-28VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-28VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 629,
    "name": "ET-Q24H-36VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-36VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 630,
    "name": "ET-Q24H-48VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24H-48VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 631,
    "name": "ET-Q24M-12VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-12VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 632,
    "name": "ET-Q24M-15VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-15VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 633,
    "name": "ET-Q24M-24VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-24VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 634,
    "name": "ET-Q24M-28VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-28VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 635,
    "name": "ET-Q24M-36VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-36VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 636,
    "name": "ET-Q24M-48VP100R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q24M-48VP100R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 637,
    "name": "ET-Q28H-05VP50R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q28H-05VP50R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 638,
    "name": "ET-Q28M-05VP50R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q28M-05VP50R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 639,
    "name": "ET-Q300H-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q300H-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 640,
    "name": "ET-Q300M-3V3P050R23",
    "category": "Преобразователи напряжения",
    "subcategory": "Модульные преобразователи ИРТЫШ",
    "description": "Модульный преобразователь DC/DC Иртыш ET-Q300M-3V3P050R23",
    "image": "../assets/images/products/items/Irtysh.webp",
    "specs": {
      "Тип": "DC/DC"
    }
  },
  {
    "id": 641,
    "name": "0R1",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,1 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "0,1 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 642,
    "name": "0R2",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,2 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "0,2 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 643,
    "name": "0R3",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,3 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "0,3 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 644,
    "name": "0R4",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,4 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "0,4 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 645,
    "name": "0R5",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,5 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "0,5 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 646,
    "name": "0R6",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,6 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "0,6 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 647,
    "name": "0R7",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,7 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "0,7 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 648,
    "name": "0R8",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,8 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "0,8 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 649,
    "name": "0R9",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 0,9 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "0,9 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 650,
    "name": "1R0",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.0 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "1.0 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 651,
    "name": "1R1",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.1 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4a0/qq6siqp1q4sokmjhpu6xhuw9wwqgjj4r/200_200_1/14.jpg",
    "specs": {
      "Ёмкость": "1.1 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 652,
    "name": "1R2",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.2 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/4da/bob3brjbqp1q8vv36aehyszrzuluftm8/200_200_1/11.jpg",
    "specs": {
      "Ёмкость": "1.2 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 653,
    "name": "1R3",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.3 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.3 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 654,
    "name": "1R4",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.4 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.4 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 655,
    "name": "1R5",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.5 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.5 пФ",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 656,
    "name": "1R6",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.6 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.6 пФ",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 657,
    "name": "1R7",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.7 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.7 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 658,
    "name": "1R8",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.8 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.8 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 659,
    "name": "1R9",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1.9 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "1.9 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 660,
    "name": "2R0",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 2.0 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "2.0 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 661,
    "name": "2R1",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 2.1 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "2.1 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 662,
    "name": "2R2",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 2.2 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "2.2 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 663,
    "name": "2R4",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 2.4 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "2.4 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 664,
    "name": "2R7",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 2.7 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "2.7 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 665,
    "name": "3R0",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 3.0 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/ccc/nhzzmw9kos8vxxft8iywuln5hey9u637/200_200_1/13.jpg",
    "specs": {
      "Ёмкость": "3.0 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 666,
    "name": "3R3",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 3.3 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "3.3 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 667,
    "name": "3R6",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 3.6 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "3.6 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 668,
    "name": "3R9",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 3.9 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "3.9 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 669,
    "name": "4R3",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 4.3 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "4.3 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 670,
    "name": "4R7",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 4.7 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "4.7 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 671,
    "name": "5R1",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 5.1 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "5.1 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 672,
    "name": "5R6",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 5.6 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "5.6 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 673,
    "name": "6R2",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 6.2 пФ",
    "image": "https://ekb-test.ru/upload/resize_cache/iblock/6e0/mw3t8an92b3l7eoiqzbbad9oastj2xxg/200_200_1/12.jpg",
    "specs": {
      "Ёмкость": "6.2 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.05 пФ … ±0.5 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 674,
    "name": "6R8",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 6.8 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "6.8 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.1 пФ … ±0.25 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 675,
    "name": "7R5",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 7.5 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "7.5 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.1 пФ … ±0.25 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 676,
    "name": "8R2",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 8.2 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "8.2 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.1 пФ … ±0.25 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 677,
    "name": "9R1",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 9.1 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "9.1 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±0.1 пФ … ±0.25 пФ",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 678,
    "name": "100",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 10 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "10 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 679,
    "name": "110",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 11 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "11 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 680,
    "name": "120",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 12 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "12 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 681,
    "name": "130",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 13 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "13 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 682,
    "name": "150",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 15 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "15 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 683,
    "name": "160",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 16 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "16 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 684,
    "name": "180",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 18 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "18 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 685,
    "name": "200",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 20 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "20 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 686,
    "name": "220",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 22 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "22 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 687,
    "name": "240",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 24 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "24 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 688,
    "name": "270",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 27 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "27 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 689,
    "name": "300",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 30 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "30 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 690,
    "name": "330",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 33 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "33 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 691,
    "name": "360",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 36 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "36 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 692,
    "name": "390",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 39 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "39 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 693,
    "name": "430",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 43 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "43 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 694,
    "name": "470",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 47 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "47 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "150 В Код 151 или 300 В Код 301",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 695,
    "name": "510",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 51 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "51 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 696,
    "name": "560",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 56 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "56 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 697,
    "name": "620",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 62 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "62 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 698,
    "name": "680",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 68 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "68 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 699,
    "name": "750",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 75 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "75 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 700,
    "name": "820",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 82 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "82 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 701,
    "name": "910",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 91 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "91 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 702,
    "name": "101",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 100 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "100 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 703,
    "name": "111",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 110 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "110 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 704,
    "name": "121",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 120 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "120 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 705,
    "name": "131",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 130 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "130 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 706,
    "name": "151",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 150 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "150 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 707,
    "name": "161",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 160 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "160 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 708,
    "name": "181",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 180 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "180 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 709,
    "name": "201",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 200 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "200 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 710,
    "name": "221",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 220 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "220 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 711,
    "name": "241",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 240 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "240 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 712,
    "name": "271",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 270 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "270 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 713,
    "name": "301",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 300 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "300 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151 или 200 В Код 201",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 714,
    "name": "331",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 330 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "330 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 715,
    "name": "361",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 360 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "360 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 716,
    "name": "391",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 390 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "390 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 717,
    "name": "431",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 430 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "430 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 718,
    "name": "471",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 470 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "470 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 719,
    "name": "511",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 510 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "510 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 720,
    "name": "561",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 560 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "560 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 721,
    "name": "621",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 620 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "620 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "151 В Код 151",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 722,
    "name": "681",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 680 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "680 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "50 В Код 500 или 100 В Код 101",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 723,
    "name": "751",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 750 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "750 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "50 В Код 500 или 100 В Код 101",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 724,
    "name": "821",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 820 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "820 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "50 В Код 500 или 100 В Код 101",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 725,
    "name": "911",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 910 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "910 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "50 В Код 500 или 100 В Код 101",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 726,
    "name": "102",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ЭКБ ТЕСТ",
    "description": "СВЧ-конденсатор ARC70A 1000 пФ",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {
      "Ёмкость": "1000 пФ",
      "Корпус": "0505",
      "Номинальное напряжение": "50 В Код 500 или 100 В Код 101",
      "Температурный диапазон": "-55…200 °C",
      "Точность": "±1% … ±5%",
      "Тип": "Сверх-высокочастотные многослойные керамические чип-конденсаторы для ВП",
      "Область применения": "<p>Модули радиотехнической разведки и радиоэлектронной борьбы для военных сверхвысокочастотных радиолокационных станций с фазированной антенной решеткой, военные коротковолновые, ультракоротковолновые, микроволновые системы связи.</p>"
    }
  },
  {
    "id": 727,
    "name": "ATC700A",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы ATC (American Technical Ceramics)",
    "description": "СВЧ-конденсаторы ATC700A (American Technical Ceramics)",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {}
  },
  {
    "id": 728,
    "name": "0505C/P",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы PPI",
    "description": "СВЧ-конденсаторы 0505C/P (PPI)",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {}
  },
  {
    "id": 729,
    "name": "DLC70A",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы DLC",
    "description": "СВЧ-конденсаторы DLC70A (DLC)",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {}
  },
  {
    "id": 730,
    "name": "К15-39",
    "category": "СВЧ-конденсаторы",
    "subcategory": "СВЧ-конденсаторы Гериконд",
    "description": "СВЧ-конденсаторы К15-39 (Гериконд)",
    "image": "../assets/images/products/items/ARC70A 1.webp",
    "specs": {}
  },
  {
    "id": 731,
    "name": "ET1008",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1 ГГц ET1008",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "90 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "18 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "28.95 × 17.78 × 4.85 мм"
    }
  },
  {
    "id": 732,
    "name": "ET1006",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1,5 ГГц ET1006",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "70 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "20 дБ",
      "Граничная частота": "1,5 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "20.30 × 10.80 × 4.23 мм"
    }
  },
  {
    "id": 733,
    "name": "ET2022",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1 ГГц ET2022",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "50 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "20 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "28.95 × 17.78 × 4.85 мм"
    }
  },
  {
    "id": 734,
    "name": "ET1028",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 250 ГГц ET1028",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "400 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "17 дБ",
      "Граничная частота": "250 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "28.95 × 17.78 × 4.85 мм"
    }
  },
  {
    "id": 735,
    "name": "ET09100",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1 ГГц ET09100",
    "image": "../assets/images/products/items/ET09100.webp",
    "specs": {
      "Мощность": "30 Вт",
      "Рабочее напряжение": "13,6 В",
      "Усиление": "14 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Пластиковый корпус поверхностного монтажа с контактными площадками",
      "Размеры корпуса": "10.00 × 6.00 × 2.10 мм"
    }
  },
  {
    "id": 736,
    "name": "ET25007",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 2,5 ГГц ET25007",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "3 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "17 дБ",
      "Граничная частота": "2,5 ГГц",
      "Тип корпуса": "Пластиковый корпус поверхностного монтажа с контактными площадками",
      "Размеры корпуса": "10.00 × 6.00 × 2.10 мм"
    }
  },
  {
    "id": 737,
    "name": "ET6004H",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 4 ГГц ET6004H",
    "image": "../assets/images/products/items/ET6004H 1.webp",
    "specs": {
      "Мощность": "35 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "16 дБ",
      "Граничная частота": "4 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "14.00 × 8.13 × 3.56 мм"
    }
  },
  {
    "id": 738,
    "name": "ET0506",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1 ГГц ET0506",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "60 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "19 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.32 × 24.76 × 6.67 мм"
    }
  },
  {
    "id": 739,
    "name": "ET0520",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 0,5 ГГц ET0520",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "200 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "20 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.32 × 24.76 × 6.67 мм"
    }
  },
  {
    "id": 740,
    "name": "ET1503V",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 1500 МГц ET1503V",
    "image": "../assets/images/products/items/ET1503V.webp",
    "specs": {
      "Мощность": "30 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "28 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.32 × 24.76 × 6.67 мм"
    }
  },
  {
    "id": 741,
    "name": "ET0535",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 500 МГц ET0535",
    "image": "../assets/images/products/items/ET0535.webp",
    "specs": {
      "Мощность": "300 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "16,2 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "34.29 × 20.00 × 5.80 мм"
    }
  },
  {
    "id": 742,
    "name": "ET1001",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 2 ГГц ET1001",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "10 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "23 дБ",
      "Граничная частота": "2 ГГц",
      "Тип корпуса": "Пластиковый корпус поверхностного монтажа SO8 с формованными выводами",
      "Размеры корпуса": "7.36 × 05.08 × 2.18 мм"
    }
  },
  {
    "id": 743,
    "name": "ET574F",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 500 МГц ET574F",
    "image": "../assets/images/products/items/ET574F.webp",
    "specs": {
      "Мощность": "700 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "17,7 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "41.15 × 16.60 × 4.45 мм"
    }
  },
  {
    "id": 744,
    "name": "ET2006C",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 2 ГГц ET2006C",
    "image": "../assets/images/products/items/ET2006C.webp",
    "specs": {
      "Мощность": "24 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "15 дБ",
      "Граничная частота": "2 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.30 × 10.80 × 4.23 мм"
    }
  },
  {
    "id": 745,
    "name": "ETM1001",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 2 ГГц ETM1001",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "10 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "2 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "7.49 × 5.16 × 2.34 мм"
    }
  },
  {
    "id": 746,
    "name": "ET2201N",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 2,2 ГГц ET2201N",
    "image": "../assets/images/products/items/ET2201N.webp",
    "specs": {
      "Мощность": "10 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "14 дБ",
      "Граничная частота": "2,2 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "7.49 × 5.16 × 2.34 мм"
    }
  },
  {
    "id": 747,
    "name": "ET3135",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы LDMOS",
    "description": "Высокочастотный транзистор 700-3500 МГц ET3135",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "20 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "от 15,4 до 22,9 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "10.29 × 6.93 × 3.38 мм"
    }
  },
  {
    "id": 748,
    "name": "D1008UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D1008UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "80 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "24.76 × 16.51 × 4.82 мм"
    }
  },
  {
    "id": 749,
    "name": "D2022UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2022UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "45 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "18.90 × 16.38 × 05.08 мм"
    }
  },
  {
    "id": 750,
    "name": "D1028UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D1028UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "300 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "175 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "34.03 × 19.05 × 05.08 мм"
    }
  },
  {
    "id": 751,
    "name": "PD55015-E",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор PD55015-E",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный LDMOS с общим истоком",
      "Мощность": "15 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "14 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Пластиковый корпус PowerSO-10RF с 2-мя формованными или прямыми выводами",
      "Размеры корпуса": "13.85 × 9.40 × 3.40 мм"
    }
  },
  {
    "id": 752,
    "name": "PD55003",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор PD55003",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Радиочастотный N-канальный LDMOS",
      "Мощность": "3 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "17 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Пластиковый корпус PowerSO-10RF с 2-мя формованными или прямыми выводами",
      "Размеры корпуса": "13.85 × 9.40 × 3.40 мм"
    }
  },
  {
    "id": 753,
    "name": "NPTB00025",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор NPTB00025",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Радиочастотный N-канальный LDMOS",
      "Мощность": "25 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13,5 дБ",
      "Граничная частота": "4 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "13.97 × 8.13 × 3.10 мм"
    }
  },
  {
    "id": 754,
    "name": "BLF171A (MRF171A)",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLF171A (MRF171A)",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Нитрид-галлиевый (GaN) радиочастотный HEMT",
      "Мощность": "45 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "17 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.32 × 20.32 × 6.67 мм"
    }
  },
  {
    "id": 755,
    "name": "MFR174",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор MFR174",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Радиочастотный N-канальный LDMOS",
      "Мощность": "125 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "9 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "26.04 × 26.04 × 6,58 мм"
    }
  },
  {
    "id": 756,
    "name": "D1002UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D1002UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "40 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "16 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "20.32 × 20.32 × 6.67 мм"
    }
  },
  {
    "id": 757,
    "name": "BLF278",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLF278",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Радиочастотный N-канальный LDMOS",
      "Мощность": "300 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "18 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "34.30 × 20 × 5.80 мм"
    }
  },
  {
    "id": 758,
    "name": "BLF404",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLF404",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "4 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "11,5 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Пластиковый корпус поверхностного монтажа SO8 с формованными выводами",
      "Размеры корпуса": "7.36 × 05.08 × 2.18 мм"
    }
  },
  {
    "id": 759,
    "name": "BLF574",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLF574",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Радиочастотный LDMOS",
      "Мощность": "600 Вт",
      "Рабочее напряжение": "50 В",
      "Усиление": "27,5 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "41.15 × 16.60 × 4.45 мм"
    }
  },
  {
    "id": 760,
    "name": "D1207UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D1207UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "20 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "10 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "18.90 × 16.38 × 05.08 мм"
    }
  },
  {
    "id": 761,
    "name": "D2001UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2001UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Тип": "Сдвоенный N-канальный радиочастотный LDMOS с общим истоком",
      "Мощность": "2,5 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 2 вывода",
      "Размеры корпуса": "18.92 × 16.51 × 6.35 мм"
    }
  },
  {
    "id": 762,
    "name": "D2202UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2202UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "5 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "10 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 2 вывода",
      "Размеры корпуса": "18.92 × 16.51 × 6.35 мм"
    }
  },
  {
    "id": 763,
    "name": "BLS6G3135",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLS6G3135",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "20 Вт",
      "Рабочее напряжение": "28 В",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "15,37 × 4,62 × 7,24 мм"
    }
  },
  {
    "id": 764,
    "name": "D2208UK (SemeLAB)",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2208UK (SemeLAB)",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "40 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "10 дБ",
      "Граничная частота": "0,5 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "24.76 × 16.51 × 6.47 мм"
    }
  },
  {
    "id": 765,
    "name": "BLF246",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор BLF246",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "80 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "16 дБ",
      "Тип корпуса": "Керамический корпус с фланцами",
      "Размеры корпуса": "27.00 × 27.00 × 6.67 мм"
    }
  },
  {
    "id": 766,
    "name": "D2213UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2213UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "20 Вт",
      "Рабочее напряжение": "12,5 В",
      "Усиление": "10 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 4 вывода",
      "Размеры корпуса": "24.76 × 16.51 × 6.47 мм"
    }
  },
  {
    "id": 767,
    "name": "D2002UK",
    "category": "СВЧ-транзисторы",
    "subcategory": "СВЧ-транзисторы других производителей",
    "description": "Высокочастотный транзистор D2002UK",
    "image": "../assets/images/products/items/0eie6w0m_LDMOS_Microwave_Transistors 1.webp",
    "specs": {
      "Мощность": "5 Вт",
      "Рабочее напряжение": "28 В",
      "Усиление": "13 дБ",
      "Граничная частота": "1 ГГц",
      "Тип корпуса": "Керамический корпус с фланцами: 2 монтажных отверстия, 2 вывода",
      "Размеры корпуса": "18.92 × 16.51 × 6.35 мм"
    }
  }
];
